...
class Arena : Entity,HasAnchoring,HasModel
{
    ...
    required init(transformComponent : Transform,movableComponent : MovableComponent,isHost : Bool,nearbyService : NearbyService,pointTracker : PointsViewModel, nickTracker : NickViewModel) {
        
        super.init()
        self.transform = transformComponent
        self.initzialize(movableComponent: movableComponent, isHost: isHost, nearbyService : nearbyService, pointTracker : pointTracker, nickTracker : nickTracker)
        
        
    }
    
    func initzialize(movableComponent : MovableComponent,isHost : Bool,nearbyService : NearbyService, pointTracker : PointsViewModel, nickTracker : NickViewModel) {
        ...
        if isHost{
            self.piattini.append(createPiattino(movableComponent: movableComponent, nickTracker: nickTracker))
            self.children.append(dischetto)
            self.goals = createGoals(pointTracker : pointTracker)
            self.children.append(contentsOf: self.goals)
        }
        else{
            self.piattini.append(createPiattinoClient(movableComponent: movableComponent, nickTracker: nickTracker))
        }
...
//        self.dischetto.addForce(.init(x: 50, y: 0, z: 0), relativeTo: self)
    ...
    func createPiattino(movableComponent: MovableComponent, nickTracker : NickViewModel) -> Piattino
    {
        ...
        let networkComponent = NetworkComponent(networkSender: nearbyService!)
        let hitSoundComponent = HitSoundComponent()
        let nicknameComponent = NickComponent(nickTracker: nickTracker, arena: self)
        let piattino = Piattino(modelComponent: modelComponent, movableComponent: movableComponent, tranform: transform,restrictionComponent: restrictionComponent,audioComponent: audioComponent,network: networkComponent,hitSound: hitSoundComponent, nickname: nicknameComponent)
        
        return piattino
    }
    
    func createPiattinoClient(movableComponent: MovableComponent, nickTracker : NickViewModel) -> Piattino
    {
